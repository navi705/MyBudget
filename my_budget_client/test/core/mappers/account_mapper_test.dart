...
void main() {
  ...
  drift.DbAccount rowFromCompanion(drift.AccountsCompanion c) => drift.DbAccount(
    id: c.id.value,
    name: c.name.value,
  ...
  test('toCompanion recomputes balanceMinor from balance+currencyCode, ignoring a stale domain.balanceMinor', () {
    final a = account(balance: 123.45, currencyCode: 'EUR', balanceMinor: 999);
  ...
  test('0-decimal fiat (JPY) scales balanceMinor with no fractional digits', () {
    final a = account(balance: 1000, currencyCode: 'JPY');
    expect(a.toCompanion().balanceMinor.value, 1000);
  });

  test('full round trip preserves every field for a fiat account with all optional fields set', () {
    ...
    expect(roundTripped.balanceMinor, 250010); // exact minor units, recomputed on write
    expect(roundTripped.currencyCode, original.currencyCode);
    expect(roundTripped.currencyDesignationId, original.currencyDesignationId);
  ...
  test('full round trip preserves every field for a crypto account (balanceMinor stays null throughout)', () {
    final original = account(
      id: 'acc-crypto',
      balance: 0.12345678,
      currencyCode: 'BTC',
  ...
  test('toCompanion with nullToAbsent:true marks null optional fields absent instead of Value(null)', () {
    final a = account(description: null, styleId: null, country: null, assetId: null, feeStructure: null);
  ...
  test('toDomain trusts the stored balanceMinor as-is (read path does not recompute)', () {
