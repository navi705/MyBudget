...
void main() {
  ...
  setUp(() async {
    syncFolder = await Directory.systemTemp.createTemp('mybudget_sync_bookkeeping_');
    ...
    // fixed.
    await db.customStatement('UPDATE sync_log SET exported = 1 WHERE exported = 0');
  });
  ...
  Future<SyncLogData?> pendingLogFor(String recordId) => (db.select(
    db.syncLog,
  )..where((t) => t.recordId.equals(recordId) & t.exported.equals(false))).getSingleOrNull();
  Future<SyncLogData?> exportedLogFor(String recordId) => (db.select(
    db.syncLog,
  )..where((t) => t.recordId.equals(recordId) & t.exported.equals(true))).getSingleOrNull();
  ...
  test('a successful export marks its rows exported and writes a file', () async {
    await addStyle('s1');
  ...
  test('a failed export (folder gone) leaves the row pending for retry', () async {
    await addStyle('s1');
    expect(await pendingLogFor('s1'), isNot(null));
  ...
  test('deleting a record produces a delete log row that is exported', () async {
    await addStyle('s1');
    await service.exportNow();
  ...
  test('importing an exported delete soft-deletes the record on the peer', () async {
  ...
  test('only rows touched by a given export are marked exported (later local writes stay pending)', () async {
    await addStyle('s1');
    await service.exportNow();
    expect(await pendingLogs(), isEmpty);
