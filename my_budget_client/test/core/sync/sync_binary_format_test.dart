...
void main() {
  group('encode/decode round trip', () {
    test(
      'single upsert change preserves table, record id, action and data',
      () {
    ...
    test('multiple changes preserve order', () {
      ...
      final packet = SyncBinaryFormat.decode(bytes);
      expect(packet.changes.map((c) => c.recordId).toList(), [
        'first',
        'second',
        'third',
      ]);
    ...
    test(
      'device id shorter than 36 bytes round trips exactly (padding trimmed)',
      () {
        final bytes = SyncBinaryFormat.encode(
          deviceId: 'abc',
          timestamp: 1,
    ...
    test(
      'record id at the 255-byte length-prefix limit round trips exactly',
      () {
        final longId = 'x' * 255;
        final bytes = SyncBinaryFormat.encode(
          deviceId: 'device-x',
  ...
  group('malformed input is rejected, not silently accepted', () {
    test('empty byte array throws rather than returning an empty packet', () {
      expect(() => SyncBinaryFormat.decode(Uint8List(0)), throwsA(anything));
    });

    test('non-gzip bytes throw', () {
      final plainText = Uint8List.fromList(
        utf8.encode('this is not gzip data'),
      );
      expect(() => SyncBinaryFormat.decode(plainText), throwsA(anything));
    });

    test(
      'gzip payload with a valid-but-wrong header throws FormatException',
      () {
        ...
        final wrongHeader = Uint8List.fromList(
          GZipEncoder().encode(
            utf8.encode('NOPE and then some padding bytes'),
          )!,
        );
        expect(
    ...
    test(
      'gzip payload shorter than the 4-byte header throws FormatException',
      () {
        final tooShort = Uint8List.fromList(
          GZipEncoder().encode([0x53, 0x59])!,
        );
        expect(
    ...
    test('unsupported version byte throws FormatException', () {
      ...
      final tampered = Uint8List.fromList(GZipEncoder().encode(decompressed)!);
    ...
    test(
      'unknown table id byte is skipped and the rest of the packet survives',
      () {
    ...
    test(
      'unknown action byte is skipped without desyncing the byte stream',
      () {
        final valid = SyncBinaryFormat.encode(
          deviceId: 'device-x',
          timestamp: 1,
    ...
    test(
      'truncated packet (change data cut short) throws rather than returning partial data',
      () {
        final valid = SyncBinaryFormat.encode(
          deviceId: 'device-x',
          timestamp: 1,
        ...
        expect(() => SyncBinaryFormat.decode(tamperedGzip), throwsA(anything));
      },
    );
  });
}
