...
void main() {
  ...
  test(
    'non-gzip garbage does not throw and leaves local data untouched',
    () async {
      await addLocalStyle('s1');
      final before = await styleIds();
  ...
  test(
    'truncated packet does not throw and leaves local data untouched',
    () async {
      await addLocalStyle('s1');
      final before = await styleIds();
  ...
  test(
    'unknown table id byte does not throw and leaves local data untouched',
    () async {
      await addLocalStyle('s1');
      final before = await styleIds();
  ...
  test(
    'a permanently malformed file is not marked processed and does not spam retries beyond the quarantine limit',
    () async {
      await writeSyncFile('peer_bad.sync', utf8.encode('garbage'));
      ...
      expect(
        ...
        isFalse,
        reason:
            'a file that only ever fails to decode must never be marked processed',
      );
    },
  );
}
